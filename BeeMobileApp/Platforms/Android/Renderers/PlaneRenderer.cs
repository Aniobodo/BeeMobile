...
{
    public class PlaneRenderer : IDisposable
    {
        ...
        public void CreateOnGlThread(Context context, string gridDistanceTextureName)
        {
            try
            {
                int vertexShader = ShaderUtil.LoadGLShader(TAG, context,
                    GLES20.GlVertexShader, _Microsoft.Android.Resource.Designer.ResourceConstant.Raw.plane_vertex);
                int passthroughShader = ShaderUtil.LoadGLShader(TAG, context,
                        GLES20.GlFragmentShader, _Microsoft.Android.Resource.Designer.ResourceConstant.Raw.plane_fragment);
        ...
        public void DrawPlanes(IEnumerable<Plane> allPlanes, Pose cameraPose, float[] cameraPerspective)
        {
            try
            {
                ...
                foreach (var sortedPlane in sortedPlanes)
                {
                    ...
                    int colorIndex = 0;//planeIndex % PLANE_COLORS_RGBA.Length;
                    ...
                    float angleRadians = 0;// planeIndex * 0.144f;
