...
{
    public class BandRenderer : IDisposable
    {
        ...
        private static readonly int numSegments = 4;
        ...
        public void setVerts(float v0, float v1, float v2, float v3, float v4, float v5, float angleBwVetor, float scale)
        {
            try
            {
                ...
                float r = 0.01f / scale;
                float nx = r * (float)Math.Sin(angleBwVetor);
                float z = r * (float)Math.Cos(angleBwVetor);
                coordinates[0] = nx + v0;
                coordinates[1] = v1;
                coordinates[2] = -z + v2;

                nx = -r * (float)Math.Sin(angleBwVetor);
                z = -r * (float)Math.Cos(angleBwVetor);
                coordinates[3] = nx + v0;
                coordinates[4] = v1;
                coordinates[5] = -z + v2;
        ...
        public void DrawBand(float[] cameraView, float[] cameraPerspective)
        {
            try
            {
